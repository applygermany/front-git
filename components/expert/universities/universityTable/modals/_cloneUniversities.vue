<template>
  <div class="clone-university-modal" id="clone-university-modal" uk-modal>
    <div class="uk-modal-dialog uk-margin-auto-vertical uk-modal-body">
      <div class="title">
        <p>ایمپورت از  {{user.firstname}} {{user.lastname}} </p>
        <button class="uk-modal-close-default" type="button" uk-close></button>
      </div>
      <div class="text">
        <p>
          آیا از ایمپورت کردن دانشگاه ها اطمینان دارید؟
        </p>
        <div class="submit-cancel">
          <button class="cancel uk-modal-close">انصراف</button>
          <button
              @click="cloneUniversities(user.id)"
...
export default {
  ...
  methods: {
    async cloneUniversities(id) {
        const cloneUniversity = await this.$axios.post(
          "v1/expert/cloneUserUniversity",
          {id: this.user.id, userid: this.userId}
      );
