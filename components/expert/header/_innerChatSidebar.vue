<template>
    <aside :class="{ active: chatSide == 'chat' }" class="inner-chat-sidebar">
        <div class="top">
            <div class="img-name">
                <div class="img">
                    <img v-if="chatInfo.img !== ''" :id="'chatroom_' + chatInfo.id" :src=chatInfo.img alt=""
                        class="user-pic" />
            ...
            <div class="return">
                <a @click="returnToList">
                    <svg class="uk-svg" fill="none" height="24" viewBox="0 0 24 24" width="24"
                        ...
                        <path d="M10.25 6.75L4.75 12L10.25 17.25" stroke="#141414" stroke-linecap="round"
                            stroke-linejoin="round" stroke-width="1.5"></path>
                        <path d="M19.25 12H5" stroke="#141414" stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="text" ref="text">
            ...
            <div>
                ...
                <template v-for="(msg, index) in messages">
                    <div :class="[msg.from == 'me' ? 'me' : 'other']" :key="index" v-if="msg.type == 'text'">
                        <p>{{ msg.msg }}</p>
                        <span>{{
                    ...
                    <div :class="[msg.from == 'me' ? 'me' : 'other']" :key="index" @click='downVoice(msg.msg, msg)'
                    ...
                    <div :class="[msg.from == 'me' ? 'me' : 'other']" :id="'key_' + index" :key="index"
                        @click='downFile(index, msg.id)' v-else-if="msg.type == 'file'">
        ...
        <div class="bottom">
            <form @submit.prevent="">
                <a @click="record(chatInfo.id)" class="records" ref="records" v-if="false && message == ''">
                    <svg class="uk-svg" fill="none" height="24" v-if="!recording" viewBox="0 0 24 24" width="24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                        ...
                        <path d="M5.75 12.75C5.75 12.75 6 17.25 12 17.25C18 17.25 18.25 12.75 18.25 12.75" stroke="#141414"
                            stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>
                        <path d="M12 17.75V19.25" stroke="#141414" stroke-linecap="round" stroke-linejoin="round"
                            stroke-width="1.5"></path>
                    </svg>
                    <div class="recording" v-else></div>
                </a>

                <a @click="sendMessage" class="sendMessage" ref="sendMessage" v-else>
                    <svg height="64pt" style="isolation: isolate; fill: var(--text-gray)" viewBox="0 0 64 64" width="64pt"
                        xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            <clipPath id="a">
                ...
                <textarea @keydown.enter="sendMessage" maxlength="255" placeholder="نوشتن پیام..."
                ...
                <a :class="'attach ' + (this.file ? 'active' : '')" @click="attachFile">
                    ...
                    <svg class="uk-svg" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                        <g data-name="Group 7826" id="Group_7826" transform="translate(0.251 -24)">
                            <rect data-name="Rectangle 3100" fill="#fff" height="24" id="Rectangle_3100" opacity="0"
                                transform="translate(-0.251 24)" width="24"></rect>
                            <path
                                ...
                                data-name="Path 13474" fill="#a5a29b" id="Path_13474"
                                transform="translate(16044.953 21440.75)"></path>
                        </g>
                    </svg>
                </a>
        ...
        <CoolLightBox :index="lightBoxIndex" :items="chatImagesFinal" @close="lightBoxIndex = null"
...
export default {
    ...
    computed: {
        ...
        chatImages() {
            console.log('images:')
            console.log(this.$store.getters["chat/chatImages"])
            return this.$store.getters["chat/chatImages"];
        },
    },
    methods: {
        onImageError() {
            document.getElementById('chatroom_' + this.chatInfo.id).src = '/user_light.svg';
            // $("#"+id).attr('src','test')
        },
        attachFile() {
        ...
        downFile(index, message_id) {
            if (this.chatImages.filter(e => e.id == message_id)[0] && this.chatImages.filter(e => e.id == message_id)[0].type == 'image') {
                let imgHolder = this.chatImages.filter(e => e.id == message_id)[0];
                ...
                let imgHolder = this.chatImages.filter(e => e.id == message_id)[0];
                a.setAttribute('href', imgHolder.img)
                a.click();
            }
        },
        downVoice(msg, res) {
        ...
        returnToList() {
            ...
            this.$store.commit("expert/Set_ChatSide", "list");
        },
        record(room_id) {
            console.log("test");
            // ورودی :
            ...
            if (this.recording == false) {
                ...
                navigator.mediaDevices.getUserMedia(audio).then((stream) => {
                    window.recordss = new MediaRecorder(stream);
                    window.stream = stream;

                    recordss.start();

                    window.recordss.ondataavailable = (event) => {
                ...
                console.log("stop vioce");
                window.recordss.stop();
                window.stream.getTracks().forEach(function (track) {
                    track.stop();
                });

                //console.log(this.data, 'before blob')
                window.data = this.data;
                //var blob = new Blob(data[0], { type: "audio/ogg; codec=opus" });
                var blob = data[0];
                ...
                reader.onloadend = () => {
                    ...
                    console.log({ reader: reader, blob: blob, data: this.data, base64: base64_encoded }, "audio")
                    window.socket.emit(
                        "private_message",
                        room_id,
                        window.myid,
    ...
    watch: {
        ...
        messages() {
            console.log(this.messages, "inner");
            setTimeout(
                () => (this.$refs.text.scrollTop = this.$refs.text.scrollHeight),
                1
            );
        ...
        chatSide(newChatSide, oldChatSide) {
            ...
            console.log(newChatSide)
            console.log(newChatSide == 'chat')
            if (newChatSide == 'chat') {
