<template>
  <aside :class="{ active: chatSide == 'list' }" class="chat-sidebar" key="boxes">
    <form @submit.prevent="">
      <input
        :value="searchInput"
        @keyup="changeSearchInput($event.target.value)"
        placeholder="جستجو"
        type="search"
      />
      <span>جستجو : </span>
...
<script>
import ChatBox from "@/components/expert/header/_chatBox.vue";
...
export default {
  ...
  computed: {
    ...
    chatRoomId() {
      ...
      return distinct.map((id) => chats.filter((chat) => chat.id == id)[0]);
    },
    openInChat() {
