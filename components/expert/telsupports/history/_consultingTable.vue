  ...
  <div class="consulting__table">
    <div class="uk-overflow-auto">
      <table class="uk-table expert-table">
        ...
        <tbody>
        <tr v-for="(telSupport, index) in telSupports" v-if="telSupport.user != null" :key="index">
          <td class="counter">{{ index + 1 }}</td>
          <td class="date">{{ telSupport.dayTelFa }}</td>
          <td class="name">
            <template v-if="telSupport.user == null">
              -----
            </template>
            <template v-else>
              <div style="display: flex;">
                <svg v-if="telSupport.user.type === 2" height="30" viewBox="0 0 30 30" width="30"
                ...
                <svg v-if="telSupport.user.type === 3" height="30" viewBox="0 0 30 30" width="30"
                     xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <style>
                      .a_2 {
                ...
                <p class="free" v-if="telSupport.user.type === 1">
                  Free
                </p>
                <nuxt-link :to="'/expert/resume/' + telSupport.user.id" style="display: flex;">
                  <a class="user-name">
                    {{ telSupport.user.firstname }}
                    {{ telSupport.user.lastname }}
                  </a>
                </nuxt-link>
              </div>
            </template>
          </td>
          <td class="date">{{ telSupport.fromTime }}</td>
          <td class="date">{{ telSupport.toTime }}</td>
          <td class="subject">{{ telSupport.advise }}</td>
        </tr>
        </tbody>
      </table>
    </div>
...
export default {
  computed: {
    telSupports() {
      return this.$store.getters["telSupports/telSupportsHistory"].telSupports;
    },
  },
};
</script>
