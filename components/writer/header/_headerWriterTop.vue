<template>
  <div class="header-expert__top">
    ...
    <div class="chat-messages-dark">
      ...
      <a @click="openSide('note')" class="messages" style="position: relative; margin-left: 10px">
...
export default {
  ...
  methods: {
    ...
    async changeDarkMode() {
      ...
      await this.$axios.get("v1/writer/changeDarkMode");
    },
    openSide(pos) {
      if (this.chatSide === "note" && pos === "note") {
        this.$store.commit("chat/Set_read");

        this.$store.commit("writer/Set_ChatSide", "");
        return;
      }

      this.$store.commit("writer/Set_ChatSide", pos);
    },
  },
};
</script>
