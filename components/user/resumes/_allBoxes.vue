<template>
  <div class="all-boxes">
    <div
      ...
      v-for="(motivation, index) in resumes"
      :key="index"
    >
      <div class="img-text">
        <div class="img">
          <img :src="motivation.image" alt="" />
        </div>
        <div class="text">
          <div class="top">
            <div class="right">
              ...
              <div uk-dropdown="mode: click">
                <ul class="uk-nav uk-dropdown-nav">
                  <li @click="showEditRequestModal(motivation.id)">
                    <a href="#edit-resume-request" uk-toggle>
                      درخواست ویرایش
                    </a>
                  </li>
                </ul>
          ...
          <div class="bottom">
            <div style="margin-top: 30px">
              ...
              <p v-if="motivation.status === 1 || motivation.status === 4 || motivation.status === 6 || motivation.status === 7 || motivation.status === 2">
                درخواست شما با موفقیت ثبت شد . لطفا طی چند روز مجددا مراجعه
                فرمایید .
              </p>
              <p v-if="motivation.status === 5">
                رزومه آماده است ! می توانیـد آن را از لینک روبرو دانلـود
                کنید .
              </p>
            ...
            <a
              @click="downloadFile(motivation.id,motivation.main_file)"
              class="download"
        ...
        <form>
          ...
          <div class="actions">
            <div
              class="attach"
              v-show="
                motivation.admin_attachment !== null &&
                motivation.admin_attachment !== ''
              "
            >
              <p>پیوست های کارشناس</p>
              <a :href="motivation.admin_attachment">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
            ...
            <div class="buttons">
              <nuxt-link :to="`/user/resumes/` + motivation.id" class="preview">
                پیش نمایش اطلاعات
              </nuxt-link>
              <a
                class="submit"
...
export default {
  ...
  created() {
    this.$store.dispatch("resume/getResumes");
  },
  computed: {
    resumes() {
      return this.$store.getters["resume/resumes"];
    },
  },
  methods: {
    async downloadFile(id,fileName) {
          try {
              ...
              const response = await this.$axios.get('v1/user/getWriterFile/'+id+'/'+lastPartOfId+'/resume', {
                  responseType: 'blob'
              });
    ...
    async updateMotivationExtra(id, extraText) {
      ...
      this.updating = true;
      try {
        await this.$store.dispatch("resume/updateResumeExtra", {
          id: id,
          extraText: extraText,
        });
      } catch (err) {
