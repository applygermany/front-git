...
export default {
  ...
  data() {
    let info = this.$store.getters["uploads/modalInfo"];
    return {
      options: {
        url: "http://httpbin.org/anything",
        acceptedFiles: info.type == 6 ? "image/jpg": "application/pdf",
      },
      fileName: null,
      fileSize: null,
      title: null,
  ...
  computed: {
    modalInfo() {
      return this.$store.getters["uploads/modalInfo"];
    },
  },
  methods: {
    ...
    selectFile() {
      ...
      fd.append("type", this.modalInfo.type);
    ...
    async uploadMandatoryFile() {
      ...
      if (this.modalInfo.type == null || this.file == null) return false;
      this.sending = true;
      this.selected = false;
      const fd = new FormData();
      fd.append("type", this.modalInfo.type);
