  ...
  <div class="form">
    <form @submit.prevent="checkCode">
      ...
      <div class="form-group submit">
        <button type="submit" v-if="sending === false">بررسی کد</button>
        <button type="button" v-else>چند لحظه صبر کنید</button>
      </div>
      <div class="form-group submit">
        <button type="button" @click="resendCode" v-if="counter === 0">
          ارسال مجدد کد
        </button>
      </div>
    </form>
...
export default {
  ...
  methods: {
    async checkCode() {
      ...
      try {
        await this.$store.dispatch("customAuth/verifyRecoveryCode", {
          mobile: this.mobile,
          code: code,
        });
      } catch (err) {
    ...
    async resendRecoveryCode() {
      try {
        await this.$store.dispatch("customAuth/sendRecoveryCode", this.mobile);
      } catch (err) {
